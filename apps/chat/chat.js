...
async function initChatSessionsFromIndexedDB() {
    ...
    _chatSessionsInitPromise = (async () => {
        ...
        showDebugMessage('Sessions 初始化中...');
        ...
        if (typeof localforage !== 'undefined') {
            try {
                ...
                showDebugMessage('persistedData: ' + (persistedData ? '有' : '無'));
                ...
                if (persistedData && persistedData.sx_chat_sessions && Array.isArray(persistedData.sx_chat_sessions) && persistedData.sx_chat_sessions.length > 0) {
                    ...
                    showDebugMessage('Sessions: ' + persistedData.sx_chat_sessions.length + ' 個, history: ' + (persistedData.sx_chat_sessions[0]?.history?.length || 0) + ' 條');
                ...
                showDebugMessage('Sessions error: ' + e.message);
            }
        }
        ...
        if (raw) {
            try {
                ...
                if (Array.isArray(parsed) && parsed.length > 0) {
                    ...
                    showDebugMessage('localStorage sessions: ' + parsed.length + ' 個');
        ...
        if (_chatSessionsCache === null) {
            _chatSessionsCache = [];
            showDebugMessage('Sessions cache: 空');
        }
        _indexedDBInitialized = true;
...
async function appendHistoryAndSession(role, content) {
    const history = getChatHistory();
    history.push({ role, content });
    await saveChatHistoryToIndexedDB(history);
    const activeId = getActiveChatId();
...
document.addEventListener('DOMContentLoaded', async () => {
        ...
        friendsList.querySelectorAll('.friend-list-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (existingSession) {
                    ...
                    setChatHistorySync(existingSession.history || []);
                    ...
                    renderChatListFromStorage();
                    setChatHistorySync(newSession.history);
                    if (chatTitleEl) chatTitleEl.innerText = charName;
                    showChatDetail();
                    renderHistory();
                }
    ...
    chatListView?.addEventListener('click', (event) => {
        ...
        if (target) {
            ...
            setChatHistorySync(target.history || []);
...
請直接開始內心獨白，不需要任何開頭或說明：`;
    ...
    const sendVoiceMessage = async () => {
        ...
        if (charReply) {
            ...
            setChatHistorySync(updatedHistory);
            if (activeId) {
                const sessions = loadChatSessions();
                const target = sessions.find(s => s.id === activeId);
                if (target) {
...
function renderHistory() {
    ...
    if (history.length === 0) {
        // 若 IndexedDB 尚未初始化完成，不要在此時插入問候語
        // 避免在真實資料尚未載入時誤判為空並覆寫
        if (!_indexedDBInitialized || !_chatHistoryIndexedDBReady) {
            console.log('[renderHistory] IndexedDB 尚未初始化完成，跳過問候語插入，等待資料載入');
            showDebugMessage('等待 IndexedDB 初始化，暫不插入問候');
            return;
        }
...
appendHistoryAndSession = async function(role, content) {
    await originalAppendHistoryAndSession(role, content);
