...
async function generateAITweets() {
  ...
  try {
    ...
    const systemPrompt = `‰Ω†ÊòØ‰∏Ä‰ΩçÂ?Ê•≠Á?Á§æÁæ§Â™íÈ??ßÂÆπ?µ‰??ÖÔ??ÖÈï∑?πÊ?ËßíËâ≤Ë®≠Â??å‰Ωø?®ËÄÖË??ØÂâµ‰ΩúÁ¨¶?à‰∫∫?©ÊÄßÊ†º?ÑÊé®?á„Ä?Ë´ã‰Ωø??${window.getAIReadableLangName?.(lang) || 'ÁπÅÈ?‰∏≠Ê?'} ?∞ÂØ´??Ëº∏Âá∫?ºÂ???JSON: {"tweets": [{"author": "‰ΩúËÄÖÂ?Á®?, "content": "?®Ê??ßÂÆπ", "likes": ?®Ê?ËÆöÊï∏, "retweets": ?®Ê?ËΩâÊé®?? "replies": ?®Ê??ûË??∏}]}`;

    let authors = [];
    if (generateUserTweets) authors.push('‰Ω?);
    if (char) authors.push(char.name);
    if (npcFollows.length > 0) authors.push(...npcFollows);

    if (authors.length === 0) {
      alert('Ë´ãËá≥Â∞ëÈÅ∏?á‰??ãË??≤Ê? NPC');
...
Ë´ãÁÇ∫‰ª•‰?‰ΩúËÄÖÂ??üÊ? 1-2 ?áÊé®?áÔ?${authors.join('??)}
Ë¶ÅÊ?Ôº?1. Á¨¶Â??ÑË??≤ÊÄßÊ†º?åË®≠ÂÆ?2. ?™ÁÑ∂?çÂÖ•‰∏ñÁ??∏Â?‰∏ñÁ?ËßÄË®≠Â?
3. ?πÊ?Á§æÁæ§Ê∞õÂ?Ë™øÊï¥Ë™ûÊ∞£?åÈ¢®??4. ÊØèÂ??®Ê? 30-100 Â≠?5. Ë™ûÊ∞£?™ÁÑ∂?ÅÊ?‰∫íÂ???
Ëº∏Âá∫ JSON ?ºÂ??Ç`;
    ...
    if (generateBtn) {
      ...
      generateBtn.textContent = 'AI ?üÊ??®Ê?';
    }
    if (fabAiBtn) {
...
function saveWorldbookMounts() {
  ...
  if (wbSaveBtn) {
    wbSaveBtn.textContent = 'Â∑≤ÂÑ≤Â≠?;
    setTimeout(() => {
      wbSaveBtn.textContent = '?≤Â??õË?Ë®≠Â?';
    }, 1200);
  }
}
...
function renderMountLists() {
  ...
  renderMountSelect(mountCharSelect, charList, activeChar, '?∏Ê?ËßíËâ≤');
  renderMountSelect(mountUserSelect, userList, activeUser, '?∏Ê??®Êà∂');
  renderNpcFollowList();
}
...
function renderNpcFollowList() {
  ...
  if (npcList.length === 0) {
    container.innerHTML = '<div class="settings-hint">Â∞öÊú™Âª∫Á? NPC</div>';
    return;
  }
...
function saveUserTweets() {
  ...
  try {
    ...
    console.warn('[twitter] localStorage ?≤Â?Â§±Ê?:', e);
  }
  try {
    sessionStorage.setItem(USER_TWEETS_KEY, data);
  } catch (e) {
    console.warn('[twitter] sessionStorage ?≤Â?Â§±Ê?:', e);
  }
}
...
function loadUserTweets() {
  ...
  try {
    ...
    console.warn('[twitter] localStorage ËÆÄ?ñÂ§±??', e);
  }
  if (!raw) {
    try {
      raw = sessionStorage.getItem(USER_TWEETS_KEY);
    } catch (e) {
      console.warn('[twitter] sessionStorage ËÆÄ?ñÂ§±??', e);
    }
  }
  if (raw) {
    try {
      ...
      console.warn('[twitter] JSON Ëß??Â§±Ê?:', e);
    }
  }
  return [];
}
...
function renderTweets() {
  ...
  if (displayTweets.length === 0) {
    feedEl.innerHTML = '<div class="empty-state">Ê≠°Ë?‰ΩøÁî®?®Áâπ</div>';
    return;
  }
  ...
  feedEl.innerHTML = displayTweets.map(tweet => {
    ...
    const isUserTweet = tweet.author === '‰Ω?;
...
function addRetweetToFeed(retweeterName, tweetContent, originalAuthor) {
  ...
  npcTweets.unshift({
    ...
    time: '?æÂú®',
    content: `ËΩâÁôº‰∫?@${originalAuthor} ?ÑÊé®?á\n${tweetContent}`,
    stats: { reply: 0, retweet: 0, like: 0 },
    timestamp: Date.now(),
    isRetweet: true,
    originalAuthor
...
function showComposeModal() {
  const content = prompt('?â‰?È∫ºÊñ∞ÈÆÆ‰?Ôº?);
...
function bindEvents() {
  ...
  replyGuidelinesSaveBtn?.addEventListener('click', () => {
    ...
    replyGuidelinesSaveBtn.textContent = 'Â∑≤ÂÑ≤Â≠?;
    setTimeout(() => {
      replyGuidelinesSaveBtn.textContent = '?≤Â?Ê≥®Ê?‰∫ãÈ?';
    }, 1200);
  });
  ...
  worldviewSaveBtn?.addEventListener('click', () => {
    ...
    worldviewSaveBtn.textContent = 'Â∑≤ÂÑ≤Â≠?;
    setTimeout(() => {
      worldviewSaveBtn.textContent = '?≤Â?‰∏ñÁ?ËßÄ';
    }, 1200);
  });
  ...
  communitySaveBtn?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      communitySaveBtn.textContent = '?≤Â?Á§æÁæ§Ë®≠Â?';
    }, 1200);
  });
...
const saveTwitterData = () => {
  try {
    ...
    console.warn('[twitter] ‰øùÂ??∏Ê?Â§±Ê?:', e);
  }
  saveUserTweets();
};
...
function executeReaction(reaction) {
  ...
  switch (type) {
    ...
    case 'reply':
      const replies = ['?ôÂÄãË?ÈªûÂ??âË∂£Ôº?, '?åÊ?Ôº?, 'Ë™™Â?Â•?, '?®‰???, '?üÁ??áÁ?Ôº?, '?àÂ?Ê≤íÈåØ', '?ë‰??ôÈ∫ºË¶∫Â?', 'Â§™ÊâØ‰∫ÜÂêß', '?üË??Ü‰∫´Ôº?, 'Â≠∏Âà∞‰∫ÜÊñ∞?±Ë•ø'];
