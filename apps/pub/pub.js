...
const defaultCharacter = {
    ...
    tags: ['default', 'chat'],
    avatar: '',
    systemPrompt: 'ä½ æ˜¯ä¸€ä½å?æ¥­ç?è§’è‰²?®æ??Šå¤©?©æ??‚ä½¿?¨ç?é«”ä¸­?‡å?è¦†ã€?
};
    ...
    state.worldbookEntries.forEach((entry, index) => {
        ...
        item.innerHTML = `
            <div class="worldbook-item-head">
                ...
                <button type="button" class="worldbook-delete" data-index="${index}">?ªé™¤</button>
            </div>
...
function renderCharacterList() {
    ...
    state.characters.forEach((char) => {
        ...
        btn.textContent = `${char.name} Â· ${(char.tags || []).join(', ')}`;
...
function updateHeader() {
    ...
    if (els.charTags) els.charTags.textContent = `@${(active.tags || []).join(' Â· ')}`;
    ...
    if (els.activeModel) els.activeModel.textContent = state.settings.model || '?ªè¨­å®šæ¨¡??;
...
function renderMessages() {
    ...
    if (!messages.length) {
        ...
        empty.textContent = 'è§’è‰²å·²å°±ç·’ï?è¼¸å…¥ç¬¬ä??¥é?å§‹ã€?;
...
function syncSettingsForm() {
    ...
    setConnState(state.settings.apiKey ? 'å·²é?ç½? : '?¢ç?');
}
...
async function requestCompletion() {
    ...
    if (apiType === 'gemini') {
        if (!state.settings.apiKey) {
            return 'å°šæœªè¨­å? API Keyï¼Œè??ˆæ–¼å·¦å´è¨­å???;
        }
        ...
        return data.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || 'æ¨¡å??ªå??³å¯é¡¯ç¤º?§å®¹??;
    }
    
    // OpenAI ?¸å®¹?¼å??–è‡ªè¨‚ç«¯é»?    if (!state.settings.apiKey) {
        return 'å°šæœªè¨­å? API Keyï¼Œè??ˆæ–¼å·¦å´è¨­å???;
    }
    ...
    // ?ªè?ç«¯é?ä½¿ç”¨å®Œæ•´ URLï¼Œå¦?‡è‡ª?•å?ä¸?/chat/completions
    ...
    return source?.choices?.[0]?.message?.content?.trim() || 'æ¨¡å??ªå??³å¯é¡¯ç¤º?§å®¹??;
}
...
async function sendMessage() {
    ...
    try {
        ...
        appendMessage('bot', `? ï? ?è?å¤±æ?ï¼?{error.message}`);
        setConnState('?¯èª¤');
    } finally {
...
async function regenLast() {
    ...
    if (!messages.some((m) => m.role === 'user')) {
        appendMessage('bot', 'æ²’æ??¯é?å¯«ç?ä½¿ç”¨?…è??¯ã€?);
        return;
    }
    if (els.sendBtn) els.sendBtn.disabled = true;
    setConnState('?å¯«ä¸?..');
    try {
        ...
        appendMessage('bot', `? ï? ?å¯«å¤±æ?ï¼?{error.message}`);
        setConnState('?¯èª¤');
    } finally {
        if (els.sendBtn) els.sendBtn.disabled = false;
    }
...
function createCharacter() {
    const name = prompt('è§’è‰²?ç¨±');
    if (!name) return;
    const tagsRaw = prompt('è§’è‰²æ¨™ç±¤ï¼ˆé€—è??†é?ï¼?, 'rp, custom') || '';
    const newChar = {
        id: uid('char'),
        name: name.trim(),
        tags: tagsRaw.split(',').map((v) => v.trim()).filter(Boolean),
        avatar: '',
        systemPrompt: `ä½ æ˜¯ ${name.trim()}ï¼Œè?ä»¥è??²å£?»å?è¦†ã€‚`
    };
