...
const state = {
  profile: {
    userName: localStorage.getItem('sx_user_name') || '‰Ω?,
...
const saveFacebookData = () => {
  try {
    ...
    console.error('‰øùÂ? Facebook ?∏Ê?Â§±Ê?:', e);
  }
};
...
function getCommunityContext() {
  ...
  const toneMap = {
    friendly: 'Á§æÁæ§Ê∞õÂ??ãÂ?Ê∫´Â?ÔºåÂ§ßÂ§öÊï∏?®Êà∂Á¶ÆË?‰∫íÂ?',
    neutral: 'Á§æÁæ§Ê∞õÂ?‰∏≠Á?Ê≠?∏∏ÔºåÊ∑∑?àÂ?Á®ÆÊ?Â∫?,
    hostile: 'Á§æÁæ§Ê∞õÂ??ÖÊªø?≠Ë≠∞ÔºåÂÆπ?ìÂ??ºÁ??∞Â??ªÊ?',
    toxic: 'Á§æÁæ§Ê∞õÂ??°Ê?ÔºåÊ??âÁΩµ‰∫∫„ÄÅÊîª?äÊÄßË?Ë´?
  };
  
  let context = `# Á§æÁæ§Ê∞õÂ?\n${toneMap[tone] || toneMap.neutral}\n`;
  
  const allowedTypes = [];
  if (flags.criticism) allowedTypes.push('?πË?Ë®ÄË´?);
  if (flags.sarcasm) allowedTypes.push('Ë´∑Âà∫?≤Ë´∑');
  if (flags.arguments) allowedTypes.push('Á≠ÜÊà∞?≠Âêµ');
  if (flags.trolling) allowedTypes.push('???ÂºïÊà∞');
  
  if (allowedTypes.length > 0) {
    context += `?ÅË®±?ÑÂÖßÂÆπÈ??? ${allowedTypes.join('??)}\n`;
  }
...
function updateAccountSelectors() {
  ...
  const userName = userProfile?.name || user.name || '‰Ω?;
...
function updateComposerAvatar() {
  ...
  if (postInput) {
    postInput.placeholder = `${accountInfo.name}ÔºåÊÉ≥Ë™™‰?‰ªÄÈ∫ºÔ?`;
  }
  if (composeInput) {
    composeInput.placeholder = `${accountInfo.name}ÔºåÊÉ≥Ë™™‰?‰ªÄÈ∫ºÔ?`;
  }
...
function updateProfileAvatars() {
  ...
  if (postInput) {
    postInput.placeholder = `${state.profile.userName || '‰Ω?}ÔºåÊÉ≥Ë™™‰?‰ªÄÈ∫ºÔ?`;
  }
  if (composeInput) {
    composeInput.placeholder = `${state.profile.userName || '‰Ω?}ÔºåÊÉ≥Ë™™‰?‰ªÄÈ∫ºÔ?`;
  }
  const drawerUserName = document.getElementById('drawer-user-name');
  if (drawerUserName) {
    drawerUserName.textContent = state.profile.userName || '‰Ω?;
  }
}
...
function renderFriendsSidebar() {
  ...
  if (allFriends.length === 0) {
    container.innerHTML = '<div class="muted-text">Â∞öÊú™?∞Â?Â•ΩÂ?</div>';
    return;
  }
...
function renderOnlineFriends() {
  ...
  const allFriends = [...state.friends, ...state.npcFriends];
  if (allFriends.length === 0) {
    container.innerHTML = '<div class="muted-text">Ê≤íÊ?Á∑ö‰?Â•ΩÂ?</div>';
    return;
  }
...
function renderStories() {
  ...
  let storiesHtml = `
    <div class="story create" id="create-story-btn">
      ...
      <div class="story-label">Âª∫Á??êÊ??ïÊ?</div>
    </div>
  `;
...
function addPostMemory(post) {
  ...
  const dateStr = `${date.getFullYear()}Âπ?{date.getMonth() + 1}??{date.getDate()}?•`;
...
function renderPosts() {
  ...
  if (displayPosts.length === 0) {
    feedEl.innerHTML = '<div class="card muted">Â∞öÁÑ°Ë≤ºÊ?ÔºåË??®Ë®≠ÂÆö‰∏≠?†ÂÖ•Â•ΩÂ??ñÁ??êË≤º??/div>';
    return;
  }
  ...
  feedEl.innerHTML = displayPosts.map(post => {
    ...
    const visibilityIcon = post.visibility === 'friends' 
      ? '<i class="fas fa-user-friends" title="Â•ΩÂ??êÂ?"></i>'
      : post.visibility === 'private'
      ? '<i class="fas fa-lock" title="?ÖËá™Â∑?></i>'
      : '';
      ...
      <article class="post card" data-post-id="${post.id}">
        ...
        <div class="post-content">
          <div class="post-header">
            ...
            <button class="icon-btn" aria-label="?¥Â?"><i class="fas fa-ellipsis-h"></i></button>
          </div>
          ...
          <div class="reaction-picker hidden" data-post-id="${post.id}">
            <button type="button" data-reaction-type="like" title="ËÆ?><i class="fas fa-thumbs-up"></i></button>
            <button type="button" data-reaction-type="love" title="?õÂ?"><i class="fas fa-heart"></i></button>
            <button type="button" data-reaction-type="care" title="?úÂ?"><i class="fas fa-hand-holding-heart"></i></button>
            <button type="button" data-reaction-type="haha" title="?àÂ?"><i class="fas fa-laugh-squint"></i></button>
            <button type="button" data-reaction-type="wow" title="??><i class="fas fa-surprise"></i></button>
            <button type="button" data-reaction-type="sad" title="?∑Â?"><i class="fas fa-sad-tear"></i></button>
            <button type="button" data-reaction-type="angry" title="?üÊ∞£"><i class="fas fa-angry"></i></button>
          </div>
          <div class="comments-section hidden" data-post-id="${post.id}">
            <div class="comments-list">${renderComments(comments)}</div>
            <div class="comment-input-row">
              <input type="text" class="comment-input" placeholder="?ôË?..." data-post-id="${post.id}">
...
function renderComments(comments) {
  ...
  return comments.slice(-10).map(c => `
    <div class="comment-item">
      ...
      <div class="comment-time">${escapeHTML(c.time || '?õÂ?')}</div>
    </div>
  `).join('');
}
...
function renderSavedPosts() {
  ...
  if (savedPosts.length === 0) {
    feedEl.innerHTML = '<div class="card muted">Â∞öÊú™?≤Â?‰ªª‰?Ë≤ºÊ?</div>';
    return;
  }
...
function renderCharFriendsList() {
  ...
  if (charList.length === 0) {
    container.innerHTML = '<div class="muted-text">Â∞öÊú™Âª∫Á?ËßíËâ≤ÔºåË??àÂà∞Ë®≠Â?Âª∫Á?</div>';
    return;
  }

  container.innerHTML = charList.map(char => {
    const name = char.name || '?™ÂëΩ??;
...
function renderNpcFriendsList() {
  ...
  if (npcList.length === 0) {
    container.innerHTML = '<div class="muted-text">Â∞öÊú™Âª∫Á? NPCÔºåË??àÂà∞Ë®≠Â?Âª∫Á?</div>';
    return;
  }

  container.innerHTML = npcList.map(npc => {
    const name = npc.name || '?™ÂëΩ??;
...
function renderWorldbookMountList() {
  ...
  const items = index.length ? index : [{ title: '?öÁî®Â∏∏Ë?Â∫? }];

  container.innerHTML = items.map(entry => {
    const name = entry.title || entry.name || '?™ÂëΩ?ç‰??åÊõ∏';
    ...
    return `
      <div class="wb-mount-item">
        ...
        <select class="wb-mount-position" data-wb-name="${escapeHTML(name)}">
          ...
          <option value="bottom" ${position === 'bottom' ? 'selected' : ''}>Âæ?/option>
        </select>
      </div>
    `;
  }).join('');
...
function saveWorldbookMounts() {
  ...
  if (saveBtn) {
    saveBtn.textContent = 'Â∑≤ÂÑ≤Â≠?;
    setTimeout(() => { saveBtn.textContent = '?≤Â??õË?Ë®≠Â?'; }, 1200);
  }
}
...
function getChatHistoryContext() {
  ...
  try {
    ...
    if (recent.length === 0) return '?°Ë?Â§©Ë???;
    const userName = localStorage.getItem('sx_user_name') || 'User';
    return recent.map(msg => {
      const role = msg.role === 'user' ? userName : 'ËßíËâ≤';
...
async function generateAIPosts() {
  ...
  if (!apiConfig?.url) {
    alert('Â∞öÊú™Ë®≠Â? APIÔºåË??àÂà∞Ë®≠Â??ÅÈù¢?çÁΩÆ');
    return;
  }
  ...
  if (generateBtn) {
    ...
    generateBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ?üÊ?‰∏?..';
  }
  ...
  try {
    ...
    if (authors.length === 0) {
      alert('Ë´ãËá≥Â∞ëÈÅ∏?á‰??ãË??üÊ?Ë≤ºÊ??ÑÂ?Ë±?);
      return;
    }
    ...
    const systemPrompt = `‰Ω†ÊòØ‰∏Ä‰ΩçÂ?Ê•≠Á?Á§æÁæ§Â™íÈ??ßÂÆπ?µ‰??ÖÔ??ÖÈï∑?πÊ?ËßíËâ≤Ë®≠Â??µ‰?Á¨¶Â?‰∫∫Áâ©?ßÊ†º??Facebook Ë≤ºÊ???Ë´ã‰Ωø??${window.getAIReadableLangName?.(lang) || 'ÁπÅÈ?‰∏≠Ê?'} ?∞Êí∞ÂØ´„Ä?Ëº∏Âá∫?ºÂ???JSON: {"posts":[{"author":"","text":"","visibility":"public|friends","like":0,"comment":0,"share":0}]}

visibility Ë™™Ê?Ôº?- public: ?¨È?Ë≤ºÊ?ÔºåÊ??â‰∫∫?ØË?
- friends: Â•ΩÂ??êÂ?Ë≤ºÊ?ÔºåÂè™?âÂ•Ω?ãÂèØË¶?
Ë´ãÁÇ∫ÊØèÂÄã‰??ÖÁ???1-2 ?áË≤º?á„ÄÇ`;

    const contextStr = `‰ΩøÁî®?? ${JSON.stringify(user, null, 2)}
${char ? `ËßíËâ≤: ${JSON.stringify(char, null, 2)}` : ''}
‰∏ñÁ??? ${worldbooks.length > 0 ? worldbooks.map(w => `??{w.title}??{w.content}`).join('\n') : '??}
?äÂ§©Ë®òÈ?: ${chatHistory}

${communityContext}

Ë¶ÅÁ??êË≤º?áÁ?‰ΩúËÄ? ${authors.map(a => a.name).join('??)}

Ë¶ÅÊ?Ôº?1. Á¨¶Â??ÑË??≤ÊÄßÊ†º?åË®≠ÂÆ?2. ?™ÁÑ∂?çÂÖ•‰∏ñÁ??∏ÂÖßÂÆ?3. ÊØèÂ?Ë≤ºÊ? 30-100 Â≠?4. Ë™ûÊ∞£?™ÁÑ∂?ÅÊ?‰∫íÂ???5. Â•ΩÂ??ÑË≤º?áÂèØ‰ª•Ë®≠ÂÆöÁÇ∫ friends ?êÂà∂
6. ?πÊ?Á§æÁæ§Ê∞õÂ?Ë™øÊï¥?ûÊ?È¢®Ê†º`;
    ...
    if (generateBtn) {
      ...
      generateBtn.innerHTML = '<i class="fas fa-wand-magic-sparkles"></i> AI ?üÊ?Ë≤ºÊ?';
    }
  }
}
...
function loadCommunitySettings() {
  ...
  communitySaveBtn?.addEventListener('click', () => {
    ...
    saveFacebookData();
    communitySaveBtn.textContent = 'Â∑≤ÂÑ≤Â≠?;
    setTimeout(() => { communitySaveBtn.textContent = '?≤Â?Á§æÁæ§Ë®≠Â?'; }, 1200);
  });
}
...
function showProfilePage(accountValue) {
  ...
  const name = profile.name || accountInfo.name || '?™Áü•';
  ...
  feedSection.innerHTML = `
    <div class="profile-page">
      ...
      <div class="profile-posts-section">
        <div class="card">
          <div class="section-title">Ë≤ºÊ?</div>
        </div>
  ...
  if (userPosts.length === 0) {
    postsList.innerHTML = '<div class="card muted">Â∞öÁÑ°Ë≤ºÊ?</div>';
  } else {
...
if (state.isCharView && state.charViewName) {
  document.title = `${state.charViewName} ?•Á??âÊõ∏`;
}
