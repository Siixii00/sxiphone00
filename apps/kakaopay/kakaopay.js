...
function bindEvents() {
  ...
  document.getElementById('paymentCodeBtn')?.addEventListener('click', openPaymentCode);
}
...
let paymentCodeTimer = null;
let paymentCodeCountdown = 60;

function openPaymentCode() {
  const overlay = document.getElementById('paymentCodeOverlay');
  if (overlay) {
    overlay.classList.remove('hidden');
    generatePaymentCodes();
    startPaymentCodeTimer();
  }
}
...
function closePaymentCode() {
  ...
  if (paymentCodeTimer) {
    clearInterval(paymentCodeTimer);
...
function generatePaymentCodes() {
  ...
  const barcodeData = `KP${timestamp}${random}`.substring(0, 20);
  const qrcodeData = `KAKAOPAY://${timestamp}/${random}`;
  
  renderBarcode(barcodeData);
  renderQRCode(qrcodeData);
  
  document.getElementById('barcodeNumber').textContent = barcodeData;
  document.getElementById('qrcodeNumber').textContent = qrcodeData;
}
...
function startPaymentCodeTimer() {
  paymentCodeCountdown = 60;
  updatePaymentCodeTimerDisplay();
  
  if (paymentCodeTimer) {
    clearInterval(paymentCodeTimer);
  }
  
  paymentCodeTimer = setInterval(() => {
    paymentCodeCountdown--;
    updatePaymentCodeTimerDisplay();
    
    if (paymentCodeCountdown <= 0) {
      generatePaymentCodes();
      paymentCodeCountdown = 60;
      updatePaymentCodeTimerDisplay();
    }
...
function updatePaymentCodeTimerDisplay() {
  const timerCount = document.getElementById('timerCount');
  const timerFill = document.getElementById('timerFill');
  if (timerCount) timerCount.textContent = paymentCodeCountdown;
  if (timerFill) timerFill.style.width = `${(paymentCodeCountdown / 60) * 100}%`;
}

document.getElementById('closePaymentCodeBtn')?.addEventListener('click', closePaymentCode);
